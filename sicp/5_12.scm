...
(display (sort
          (map car (gcd-machine 'get-instruction-sequence))
          (lambda (x y)
...
(display (string-append "\n" "--------" "\n"))
(define (get-reg-assign reg)
  (map cddr
       (filter
        (lambda (inst)
          (and (eq? (car inst) 'assign)
               (eq? (cadr inst) reg)))
        (map car (gcd-machine 'get-instruction-sequence)))))
(display (get-reg-assign 'n))
(display (string-append "\n" "--------" "\n"))
(display (get-reg-assign 'val))