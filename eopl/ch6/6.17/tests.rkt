...
(require "cps-out-lang.rkt")      ; for scan&parse
(require "cps.rkt")              ; for cps transformer
(require "interp-register.rkt")           ; for value-of-program
...
(define run
  (lambda (string)
    (when (instrument-cps) (eopl:printf "start-to-run ~s~%" string))
    (let ((cpsed-pgm (cps-out-scan&parse string)))
