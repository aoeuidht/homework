...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (newarray-exp
       (exp1 exp2)
       (let ((arr-len (expval->num (value-of exp1 env)))
             (arr-init-val (value-of exp2 env))
             )
         (arr-val (make-array arr-len arr-init-val))))

      (arrayset-exp
       (exp-arr exp-idx exp-val)
       (let ((arr (expval->arr (value-of exp-arr env)))
             (idx (expval->num (value-of exp-idx env)))
             (val (value-of exp-val env)))
         (begin (arrayset arr idx val)
                (num-val 86))
         ))
