    ...
    (cases expression exp
      ...
      (call-exp (rator rand)
                (let ((proc (expval->proc (value-of rator env)))
                  ...
                  (eopl:printf "the arg are: ~s~%" proc)
...
(define apply-procedure
  (lambda (proc1 val)
    (when (instrument-let)
          (eopl:printf "proc ~s ~%" proc1)
          (eopl:printf "val ~s ~%" val)
          )
    (cases proc proc1
      (procedure (var body saved-env)
                 (let ((new-env (extend-env var (car val) saved-env)))
                   (when (instrument-let)
                     (eopl:printf
                     ...
                     (pretty-print (store->readable (get-store-as-list)))
                     (eopl:printf "~%"))
                   (let ((rst (value-of body new-env))
                         (param-addr (cdr val)))
                     (when (pair? param-addr)
                           (setref! (car param-addr) (deref (car val))))
                     rst
                     ))))))
...
(define value-of-operand
  (lambda (exp env)
    (let ((val (newref (value-of exp env))))
      (cases expression exp
             (var-exp (var) (list val (apply-env env var)))
             (else
              (list val)
              )))))
